   ...
   stages {
    ...
    string(credentialsId: 'IdentityConnection', variable: 'IdentityConnection'),
      ...
      container("kaniko") {
           ...
           sh """
              /kaniko/executor --context git://$GitHub_PSW@github.com/839928622/build-image-with-kaniko.git --build-arg StoreConnection=$StoreConnection --build-arg IdentityConnection=$IdentityConnection --destination ${REGISTRY}:latest --destination ${REGISTRY}:${env.BRANCH_NAME.toLowerCase()}-${BUILD_NUMBER}
             """
        }
}
